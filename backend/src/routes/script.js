...
module.exports = (sequelize) => {
  ...
  router.post('/generate', authMiddleware, usageLimitsMiddleware, async (req, res) => {
    try {
      ...
      if (!user.apiKeys?.geminiKey) {
        return res.status(400).json({ message: 'กรุณาตั้งค่า Gemini API Key ในการตั้งค่า' });
      }
      ...
      res.status(201).json({
        message: 'สร้างสคริปต์สำเร็จ',
        script
      });
    } catch (error) {
      console.error('Script generation error:', error);
      res.status(500).json({
        message: 'เกิดข้อผิดพลาดในการสร้างสคริปต์',
        error: error.message
      });
    }
  });
  ...
  router.get('/list', authMiddleware, async (req, res) => {
    try {
      ...
      res.status(500).json({ message: 'เกิดข้อผิดพลาดในการดึงข้อมูลสคริปต์' });
    }
  });
  ...
  router.delete('/:id', authMiddleware, async (req, res) => {
    try {
      ...
      res.json({ message: 'ลบสคริปต์สำเร็จ' });
    } catch (error) {
      console.error('Delete script error:', error);
      res.status(500).json({ message: 'เกิดข้อผิดพลาดในการลบสคริปต์' });
    }
  });
